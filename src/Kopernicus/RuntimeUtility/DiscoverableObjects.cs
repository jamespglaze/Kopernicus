        ...
        public void SpawnAsteroid(Asteroid asteroid, UInt32 seed)
        {
            ...
            if (Versioning.version_minor > 9)
            {
                ...
                /*else
                {
                    ...
                    VesselType cometType = VesselType.SpaceObject;
                    Orbit cometOrbit = orbit;
                    ConfigNode[] cometDef = new[]
                            {
                    ProtoVessel.CreatePartNode(
                        "PotatoComet",
                        seed
                    ...
                    ConfigNode configNode = cometDef[0].CreateVesselNode(optimizedCollider, fragmentDynamicPressureModifier, hasName: false);
                    ...
                    vessel = ProtoVessel.CreateVesselNode(asteroidName.Replace("Ast.", "Comet"),, VesselType.SpaceObject, cometOrbit, 0, new ConfigNode[1] { configNode2 }, new ConfigNode("ACTIONGROUPS"), ProtoVessel.CreateDiscoveryNode(DiscoveryLevels.Presence, randomObjClass, lifetime, maxLifetime), configNode3);
                    OverrideNode(ref vessel, asteroid.Vessel);
                    ProtoVessel protoVessel = new ProtoVessel(vessel, HighLogic.CurrentGame);
                    Kopernicus.Events.OnRuntimeUtilitySpawnAsteroid.Fire(asteroid, protoVessel);
                    protoVessel.Load(HighLogic.CurrentGame.flightState);
