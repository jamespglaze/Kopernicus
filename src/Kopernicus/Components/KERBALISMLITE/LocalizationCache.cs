...
>> #KERBALISMLITE_Mycategory_MyField = Science
...
This will autogenerate the following line for the en_us config :
>> #KERBALISMLITE_Mycategory_MyField = Getting <<1>> at <<2>>
And you can get it from the code with :
...
>> [KSPField(guiActive = true, guiName = "#KERBALISMLITE_MyModule_MyFieldName")]
...
namespace KERBALISMLITE
{
	public static class LocalHelpers
	{
		...
		public static void GenerateLoc(bool template = false)
		{
			...
			foreach (string line in localCachelines)
			{
				...
				{
					if (!template)
						locFile.Add("    #KERBALISMLITE_" + locMatch.Groups[1].Value + " = " + locMatch.Groups[2].Value);
					else
						locFile.Add("    #KERBALISMLITE_" + locMatch.Groups[1].Value + " = " + "// \"" + locMatch.Groups[2].Value + "\"");
				...
				{
					if (!template)
						locFile.Add("    #KERBALISMLITE_" + parameterLocMatch.Groups[1].Value + " = " + parameterLocMatch.Groups[2].Value);
					else
						locFile.Add("    #KERBALISMLITE_" + parameterLocMatch.Groups[1].Value + " = " + "// \"" + parameterLocMatch.Groups[2].Value + "\"");
					continue;
				}
		...
		public static void RegenerateNonEnglishLoc()
		{
			...
			{
				// #KERBALISMLITE_(.*?) = (.*?)(?=//|$)
				Match locMatch = Regex.Match(line, "#KERBALISMLITE_(.*?) = (.*?)(?=//|$)");
				if (locMatch.Success)
				{
			...
			outputLoc.Add("// IF YOU CAN BUILD KERBALISMLITE, SEE LocalizationCache.cs TO GENERATE THE MISSING STRINGS");
			outputLoc.Add("//////////////////////////////////////////////////////////////////////////////////////");
			...
			foreach (string line in englishLocLines)
			{
				...
				if (locMatch.Success)
				{
					...
					outputLoc.Add("    #KERBALISMLITE_" + locMatch.Groups[1].Value + " = " + loc);
				...
				if (parameterLocMatch.Success)
				{
					...
					outputLoc.Add("    #KERBALISMLITE_" + parameterLocMatch.Groups[1].Value + " = " + loc);
	...
	public static class Local
	{
		...
		private const string prefix = "#KERBALISMLITE_";
