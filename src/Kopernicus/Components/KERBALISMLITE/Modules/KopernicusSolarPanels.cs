...
{
	// TODO : KopernicusSolarPanels missing features :
	...
	public class KopernicusSolarPanels : PartModule
	{
		...
		[KSPField(guiActive = true, guiActiveEditor = false, guiName = "#KERBALISMLITE_SolarPanelFixer_Solarpanel")]//Solar panel
		public string panelStatus = string.Empty;

		[KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true, guiName = "#KERBALISMLITE_SolarPanelFixer_Solarpaneloutput")]//Solar panel output
		[UI_Toggle(enabledText = "#KERBALISMLITE_SolarPanelFixer_simulated", disabledText = "#KERBALISMLITE_SolarPanelFixer_ignored")]//<color=#00ff00>simulated</color>""<color=#ffff00>ignored</color>
		...
		/// If defined in the KopernicusSolarPanels config, the target module curve will be overriden.
		/// </summary>
		[KSPField(isPersistant = true)]
		...
		[KSPEvent(active = true, guiActive = true, guiName = "#KERBALISMLITE_SolarPanelFixer_Selecttrackedstar")]//Select tracked star
		...
		public override void OnLoad(ConfigNode node)
		{
			...
			// note : this rely on the fact that the reliability module is disabling the KopernicusSolarPanels monobehavior from OnStart, after OnLoad has been called
			if (!isEnabled)
			{
		...
		public static void BackgroundUpdate(Vessel v, ProtoPartModuleSnapshot m, KopernicusSolarPanels prefab, VesselData vd, ResourceInfo ec, double elapsed_s)
		{
		...
		{
			// handle the possibility of multiple solar panel and KopernicusSolarPanels modules on the part
			List<KopernicusSolarPanels> fixerModules = new List<KopernicusSolarPanels>();
			foreach (PartModule pm in part.Modules)
			{
				if (pm is KopernicusSolarPanels fixerModule)
			...
			foreach (PartModule pm in part.Modules)
			{
				...
				switch (pm.moduleName)
				{
					...
					case "KopernicusSolarPanel":
						Debug.LogWarning("Part '" + part.partInfo.title + "' use the KopernicusSolarPanel module, please remove it from your config. It is obsolete in this context.");
						continue;
					default:
		...
		public static void ProtoToggleState(KopernicusSolarPanels prefab, ProtoPartModuleSnapshot protoModule, PanelState currentState)
		{
			switch (currentState)
			{
		...
		{
			/// <summary>Reference to the KopernicusSolarPanels, must be set from OnLoad</summary>
			protected KopernicusSolarPanels fixerModule;

			/// <summary>Reference to the target module</summary>
			public abstract PartModule TargetModule { get; }

			/// <summary>
			/// Will be called by the KopernicusSolarPanels OnLoad, must set the partmodule reference.
			/// GetState() must be able to return the correct state after this has been called
			/// </summary>
			public abstract void OnLoad(KopernicusSolarPanels fixerModule, PartModule targetModule);
		...
		private class StockPanel : SupportedPanel<ModuleDeployableSolarPanel>
		{
			...
			public override void OnLoad(KopernicusSolarPanels fixerModule, PartModule targetModule)
			{
