	...
	public static class Background
	{
		...
		public static Module_type ModuleType(string module_name)
		{
			switch (module_name)
			{
				...
				case "SolarPanelFixer": return Module_type.KopernicusSolarPanels;
			}
		...
		public static void Update(Vessel v, VesselData vd, VesselResources resources, double elapsed_s)
		{
			...
			foreach (var e in Background_PMs(v))
			{
				switch(e.type)
				{
					...
					case Module_type.KopernicusSolarPanels: KopernicusSolarPanels.BackgroundUpdate(v, e.m, e.module_prefab as KopernicusSolarPanels, vd, ec, elapsed_s); break;
				}
			}
		}
		...
		private static List<BackgroundPM> Background_PMs(Vessel v)
		{
			...
			foreach (ProtoPartSnapshot p in v.protoVessel.protoPartSnapshots)
			{
				...
				{
					// TODO : this is to migrate pre-3.1 saves using WarpFixer to the new KopernicusSolarPanels. At some point in the future we can remove this code.
		...
		*/
		...
		static void MigrateWarpFixer(Vessel v, Part prefab, ProtoPartSnapshot p, ProtoPartModuleSnapshot m)
		{
			...
			KopernicusSolarPanels.PanelState state = KopernicusSolarPanels.PanelState.Unknown;
			...
			if (panelState == ModuleDeployablePart.DeployState.BROKEN)
				state = KopernicusSolarPanels.PanelState.Broken;
			else if (!panelModule.isTracking)
			{
				state = KopernicusSolarPanels.PanelState.Static;
			}
			else
			{
				switch (panelState)
				{
					case ModuleDeployablePart.DeployState.EXTENDED:
						if (!panelModule.retractable)
							state = KopernicusSolarPanels.PanelState.ExtendedFixed;
						else
							state = KopernicusSolarPanels.PanelState.Extended;
						break;
					case ModuleDeployablePart.DeployState.RETRACTED: state = KopernicusSolarPanels.PanelState.Retracted; break;
					case ModuleDeployablePart.DeployState.RETRACTING: state = KopernicusSolarPanels.PanelState.Retracting; break;
					case ModuleDeployablePart.DeployState.EXTENDING: state = KopernicusSolarPanels.PanelState.Extending; break;
					default: state = KopernicusSolarPanels.PanelState.Unknown; break;
				}
			}
